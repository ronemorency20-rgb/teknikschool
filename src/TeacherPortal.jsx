...
import {
  ...
  fetchChapters, addChapter, renameChapter, updateChapterSubtitle, deleteChapter,
  ...
  fetchCourseDocuments, uploadCourseDocument, deleteDocument,
} from "./coursesApi";
...
export default function TeacherPortal({ onProfile }) {
  ...
  const handleChapterSubtitle = async (chId, subtitle) => {
    setChapters((prev) => prev.map((c) => (c.id === chId ? { ...c, subtitle } : c))); // optimistic
    try { await updateChapterSubtitle(chId, subtitle); } catch (e) { setErr(e.message); }
  };
      ...
      <div style={{ padding: 28, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        {!loading && !sel && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <h2 style={{ margin: 0, fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Mes cours</h2>
        ...
        {!loading && sel && course && (
          <>
            ...
            <h2 style={{ margin: "0 0 4px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(26px, 6vw, 34px)", lineHeight: 1.2, overflowWrap: "break-word" }}>{course.title}</h2>
            ...
            <div style={{ background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 14, overflow: "hidden", marginBottom: 20 }}>
              ...
              <div style={{ maxHeight: 380, overflowY: "auto" }}>
                {chapters.map((ch) => (
                  <div key={ch.id} style={{ padding: "12px 18px", borderBottom: "1px solid #EDEFF3" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <input defaultValue={ch.title} onBlur={(e) => handleRenameChapter(ch.id, e.target.value)} placeholder="Titre du chapitre" style={{ flex: 1, background: "none", border: "none", color: "#12141C", fontWeight: 600, fontSize: 14, outline: "none", fontFamily: "inherit" }} />
                    ...
                    <input defaultValue={ch.subtitle || ""} onBlur={(e) => handleChapterSubtitle(ch.id, e.target.value)} placeholder="Sous-titre (optionnel) — ex. Les bases de l'ordinateur"
                      style={{ marginLeft: 44, marginTop: 4, width: "calc(100% - 44px)", background: "none", border: "none", color: "#9CA3AF", fontSize: 12, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
                  </div>
                ))}
              </div>
            </div>
...
function TeacherDocuments({ courseId }) {
  ...
  const load = async () => {
    setLoading(true);
    try { setDocs(await fetchCourseDocuments(courseId)); }
    catch (e) { setErr(e.message); }
    finally { setLoading(false); }
  };
  ...
  const handleAdd = async () => {
    ...
    setBusy(true); setErr("");
    try {
      await uploadCourseDocument(courseId, file, title.trim(), 0);
  ...
  return (
    <div style={{ background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 14, padding: 18, marginTop: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <h3 style={{ margin: 0, fontFamily: "'Syne',sans-serif", fontSize: 15 }}>Documents du cours (bibliothèque)</h3>
        <Btn onClick={() => setShowAdd((v) => !v)} small color="#7C6FFF" style={{ fontFamily: "inherit" }}>{showAdd ? "Annuler" : "+ Ajouter"}</Btn>
      </div>
      <p style={{ margin: "0 0 14px", fontSize: 12, color: "#6B7280" }}>Livres, fichiers 3D (STL), images, ou tout autre format — visibles dans la bibliothèque des élèves inscrits.</p>
      {err && <p style={{ color: "#FF6677", fontSize: 12 }}>{err}</p>}
      {showAdd && (
        <div style={{ background: "#F7F8FA", borderRadius: 10, padding: 14, marginBottom: 14 }}>
          <Inp label="Titre du document" value={title} onChange={setTitle} placeholder="ex. Guide de référence" />
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", marginBottom: 5, fontSize: 12, color: "#6B7280", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Fichier (tout format)</label>
            <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} style={{ color: "#12141C", fontSize: 13 }} />
          </div>
          <Btn onClick={handleAdd} disabled={busy || !file || !title.trim()} color="#00D4AA" style={{ fontFamily: "inherit" }}>{busy ? "Envoi…" : "Ajouter à la bibliothèque"}</Btn>
        </div>
      )}
      {loading && <Spinner size={16} label="Chargement…" />}
      {!loading && docs.length === 0 && <p style={{ color: "#9CA3AF", fontSize: 13 }}>Aucun document pour le moment.</p>}
      {!loading && docs.map((d) => (
        <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "#F7F8FA", borderRadius: 8, marginBottom: 6 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#12141C" }}>{d.title}</div>
          <button onClick={() => handleDelete(d.id)} style={{ background: "none", border: "none", color: "#FF6677", cursor: "pointer", fontSize: 12 }}>Retirer</button>
        </div>
      ))}
    </div>
  );
