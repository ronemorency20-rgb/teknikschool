...
import { fetchAllCourses, fetchStudentCourses, enrollStudent, fetchChapters, fetchChapterProgress, fetchMyCertificates, fetchAllPrerequisites, fetchUnlockedChapters, fetchChapterScores } from "./coursesApi";
...
import { SearchIcon, GraduationCapIcon, TrophyIcon, BookIcon, ChatIcon, FileTextIcon, FlagIcon, LockIcon, CheckCircleIcon } from "./Icons";
import ScoreBadge from "./ScoreBadge";
import ChapterResultsPanel from "./ChapterResultsPanel";
...
export default function StudentPortal({ onProfile }) {
  ...
  const [unlocked, setUnlocked] = useState({}); // { chapterId: boolean }
  const [scores, setScores] = useState({}); // { chapterId: { bestPct, latestPct, attempts, passed, passingScore } }
  const [resultsPanelChapter, setResultsPanelChapter] = useState(null);
  ...
  const openCourse = async (c) => {
    ...
    try {
      ...
      setChapters(ch.filter((chap) => chap.published !== false)); // hide teacher drafts from students
      setProgress(prog);
      try { setUnlocked(await fetchUnlockedChapters(c.id)); } catch { setUnlocked({}); }
      try { setScores(await fetchChapterScores(c.id, user.id)); } catch { setScores({}); }
  ...
  const refreshProgress = async () => {
    if (!selCourse) return;
    try {
      setProgress(await fetchChapterProgress(selCourse.id, user.id));
      const newUnlocked = await fetchUnlockedChapters(selCourse.id);
      const justUnlocked = chapters.find((c) => unlocked[c.id] === false && newUnlocked[c.id] === true);
      setUnlocked(newUnlocked);
      setScores(await fetchChapterScores(selCourse.id, user.id));
      if (justUnlocked) {
        setUnlockToast(justUnlocked.title);
  ...
  const handleAdvanceChapter = () => {
    const nextChapter = chapters.find((c) => c.number === selChapter.number + 1);
    if (!nextChapter) return;
    localStorage.setItem(`teknik_step_${nextChapter.id}`, "0"); // always open the newly unlocked chapter at step 1
    setSelChapter(nextChapter);
  };

  const TABS = [
    { id: "catalog", label: "Catalogue", icon: <SearchIcon size={16} /> },
    { id: "learning", label: "Mon apprentissage", icon: <GraduationCapIcon size={16} /> },
    { id: "library", label: "Bibliothèque", icon: <BookIcon size={16} /> },
  ...
  const CourseCard = ({ c, clickable }) => {
    ...
    return (
      <div
        ...
        style={{ background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 16, overflow: "hidden", cursor: clickable && enrolled ? "pointer" : "default", boxShadow: "0 2px 10px rgba(20,20,40,0.05)" }}
      >
        {c.cover_image_url ? (
        ...
        <div style={{ padding: 20 }}>
          ...
          <h3 style={{ margin: "0 0 6px", fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 16 }}>{c.title}</h3>
          <p style={{ margin: "0 0 12px", fontSize: 13, color: "#6B7280", fontFamily: "'Inter',sans-serif" }}>{c.description}</p>
      ...
      <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'DM Sans',sans-serif", color: "#12141C", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: 28, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
          <ChapterViewer
            ...
            isLastChapter={selChapter.number === Math.max(...chapters.map((c) => c.number))}
          />
        </div>
      </div>
    );
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'DM Sans',sans-serif", color: "#12141C", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: 28, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
          ...
          <div className="chapter-eyebrow" style={{ color: "#7C6FFF", textTransform: "uppercase" }}>{selCourse.price === 0 ? "Cours gratuit" : fmt(selCourse.price)}</div>
          <h2 className="course-detail-title" style={{ color: "#12141C" }}>{selCourse.title}</h2>
          <p style={{ margin: "0 0 20px", color: "#6B7280", fontSize: 13, fontFamily: "'Inter',sans-serif" }}>Durée : {selCourse.duration_days} jours</p>
          <style>{`
            ...
            .course-detail-title { font-family: 'Poppins', sans-serif; font-size: clamp(32px, 8vw, 52px); line-height: 1.05; font-weight: 800; letter-spacing: -0.025em; overflow-wrap: anywhere; margin: 0 0 16px; }
          `}</style>
              ...
              {!chLoading && (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  {chapters.map((ch) => {
                    ...
                    const isUnlocked = unlocked[ch.id] !== false; // default open until we know otherwise
                    const clickable = hasContent && isUnlocked;
                    const scoreInfo = scores[ch.id]; // undefined = never attempted
                    const stepCount = ch.chapter_content?.length || 0;

                    let badgeState = "start";
                    if (!isUnlocked) badgeState = "locked";
                    else if (scoreInfo) badgeState = "score";

                    let statusText;
                    if (!isUnlocked) statusText = "Terminez le chapitre précédent.";
                    else if (scoreInfo) statusText = `${stepCount} étape${stepCount > 1 ? "s" : ""} · Quiz ${scoreInfo.passed ? "réussi" : "échoué"}`;
                    else if (hasContent) statusText = `${stepCount} étape${stepCount > 1 ? "s" : ""} · À commencer`;
                    else statusText = "Aucun contenu";

                    return (
                      <div key={ch.id}
                        onClick={clickable ? () => setSelChapter(ch) : undefined}
                        className="chapter-card-row"
                        style={{ background: "#FFFFFF", border: `1px solid ${done ? "#00D4AA33" : "#E2E5EB"}`, borderRadius: 12, overflow: "hidden", cursor: clickable ? "pointer" : "default", opacity: isUnlocked ? 1 : 0.65 }}
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", flexWrap: "wrap", minHeight: 44 }}>
                          <div style={{ width: 32, height: 32, borderRadius: 8, background: done ? "#00D4AA22" : "#FFFFFF", border: `1px solid ${done ? "#00D4AA55" : "#E2E5EB"}`, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 12, color: done ? "#00D4AA" : "#6B7280", flexShrink: 0, fontFamily: "'Inter',sans-serif" }}>
                            {done ? "✓" : !isUnlocked ? <LockIcon size={13} color="#9CA3AF" /> : String(ch.number).padStart(2, "0")}
                          </div>
                          <div style={{ flex: "1 1 160px", minWidth: 0 }}>
                            <div style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 700, fontSize: 14, lineHeight: 1.35, overflowWrap: "anywhere" }}>{ch.title}</div>
                            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: !isUnlocked ? "#9CA3AF" : "#6B7280", marginTop: 3 }}>{statusText}</div>
                          </div>
                          <ScoreBadge
                            pct={scoreInfo ? scoreInfo.bestPct : null}
                            passed={scoreInfo?.passed}
                            state={badgeState}
                            onClick={badgeState === "score" ? (e) => { e.stopPropagation(); setResultsPanelChapter(ch); } : undefined}
                          />
                        </div>
                      </div>
                    );
                ...
                {unlockToast && (
                  <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: "#12141C", color: "#fff", padding: "12px 22px", borderRadius: 30, fontSize: 13, fontWeight: 700, boxShadow: "0 10px 30px rgba(0,0,0,.25)", zIndex: 500, display: "flex", alignItems: "center", gap: 8 }}>
                    <CheckCircleIcon size={15} color="#00D4AA" /> Chapitre suivant déverrouillé !
                  </div>
                )}

                {resultsPanelChapter && (
                  <ChapterResultsPanel
                    chapter={resultsPanelChapter}
                    studentId={user.id}
                    onClose={() => setResultsPanelChapter(null)}
                    onReview={() => { setSelChapter(resultsPanelChapter); setResultsPanelChapter(null); }}
                    onRetake={() => { setSelChapter(resultsPanelChapter); setResultsPanelChapter(null); }}
                  />
                )}
                </>
              )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'DM Sans',sans-serif", color: "#12141C", display: "flex", flexDirection: "column" }}>
      ...
      <Diaporama />
      <div className="catalog-container" style={{ maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        <style>{`.catalog-container { padding: 18px 20px calc(24px + env(safe-area-inset-bottom)); } @media (min-width: 640px) { .catalog-container { padding: 28px 28px calc(28px + env(safe-area-inset-bottom)); } }`}</style>
        ...
        {!loading && tab === "catalog" && (
          <>
            <h2 className="page-heading" style={{ margin: "0 0 8px", color: "#12141C" }}>Explorez nos cours</h2>
            <p style={{ margin: "0 0 18px", color: "#6B7280", fontSize: 14, fontFamily: "'Inter',sans-serif" }}>Apprenez à votre rythme grâce à des cours clairs, pratiques et gratuits.</p>
            <style>{`.page-heading { font-family: 'Poppins', sans-serif; font-weight: 700; font-size: clamp(32px, 8vw, 36px); line-height: 1.1; }`}</style>
        ...
        {!loading && tab === "learning" && (
          <>
            <h2 style={{ margin: "0 0 24px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Mon apprentissage</h2>
        ...
        {!loading && tab === "library" && <Library studentId={user.id} />}

        {!loading && tab === "certificates" && (
          <>
            <h2 style={{ margin: "0 0 24px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Mes certificats</h2>
