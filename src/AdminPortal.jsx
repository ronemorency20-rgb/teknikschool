...
import { deleteUserCompletely, fetchAnnouncements, createAnnouncement, deleteAnnouncement, setUserStatus, fetchDiaporamaSlides, uploadDiaporamaImage, createDiaporamaSlide, deleteDiaporamaSlide, fetchRecentActivity, fetchMessageReports, resolveReport, deleteMessage, adminSendPasswordReset, fetchUserEmailsForAdmin } from "./coursesApi";
import { UsersIcon, EyeIcon, ClipboardIcon, MegaphoneIcon, ImageIcon, BookIcon, ChartIcon, GraduationCapIcon, FileTextIcon, CheckCircleIcon, ChatIcon, FlagIcon, TrashIcon, KeyIcon } from "./Icons";
...
export default function AdminPortal({ onProfile }) {
  ...
  const load = async () => {
    ...
    try {
      const [{ data: u, error: uErr }, { data: c, error: cErr }, { data: apps, error: aErr }, ann, sl, rep, emailMap] = await Promise.all([
        ...
        fetchMessageReports(),
        fetchUserEmailsForAdmin().catch(() => ({})), // non-fatal if it fails — email button just won't work yet
      ]);
      if (uErr) throw uErr;
      if (cErr) throw cErr;
      if (aErr) throw aErr;
      setUsers((u || []).map((x) => ({ ...x, email: emailMap[x.id] })));
  ...
  const handleSendReset = async (email, name) => {
    if (!window.confirm(`Envoyer un lien de réinitialisation de mot de passe à ${name} ?`)) return;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F8FA", fontFamily: "'DM Sans',sans-serif", color: "#12141C", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: 28, maxWidth: 980, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        {!loading && tab === "users" && (
          <>
            <h2 style={{ margin: "0 0 24px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Gérer les utilisateurs</h2>
                  ...
                  <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: 14, background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 12, marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#6B7280", display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", marginTop: 2 }}>
                        ...
                        <span>·</span>
                        <span>Statut: <b style={{ color: u.status === "suspended" ? "#FF6677" : "#00D4AA" }}>{u.status === "suspended" ? "Banni" : "Actif"}</b></span>
                        {u.subject && <><span>·</span><span>{u.subject}</span></>}
                      </div>
                    </div>
                    <Badge label={u.role} color={RC[u.role]} />
                    <button onClick={() => handleSendReset(u.email, u.name)} disabled={resettingId === u.id} title="Envoyer un lien de réinitialisation de mot de passe" style={{ background: "#7C6FFF18", border: "1px solid #7C6FFF40", color: "#7C6FFF", borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
                      <KeyIcon size={12} color="#7C6FFF" /> {resettingId === u.id ? "…" : "Mot de passe"}
                    </button>
                    <button onClick={() => toggleStatus(u)} style={{ background: u.status === "suspended" ? "#00D4AA22" : "#FFB34722", border: `1px solid ${u.status === "suspended" ? "#00D4AA55" : "#FFB34755"}`, color: u.status === "suspended" ? "#00D4AA" : "#FFB347", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>
                      {u.status === "suspended" ? "Réactiver" : "Bannir"}
                    </button>
        ...
        {tab === "activity" && (
          <>
            <h2 style={{ margin: "0 0 8px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Activité des élèves</h2>
        ...
        {tab === "reports" && (
          <>
            <h2 style={{ margin: "0 0 8px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Signalements</h2>
        ...
        {!loading && tab === "applications" && (
          <>
            <h2 style={{ margin: "0 0 8px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Candidatures professeurs</h2>
        ...
        {!loading && tab === "announcements" && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <h2 style={{ margin: 0, fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Annonces</h2>
        ...
        {!loading && tab === "diaporama" && (
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <h2 style={{ margin: 0, fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Diaporama</h2>
        ...
        {!loading && tab === "courses" && (
          <>
            <h2 style={{ margin: "0 0 24px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Tous les cours</h2>
        ...
        {!loading && tab === "stats" && (
          <>
            <h2 style={{ margin: "0 0 24px", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: "clamp(22px, 5vw, 28px)" }}>Statistiques de la plateforme</h2>
