...
export async function updateChapterSubtitle(chapterId, subtitle) {
  const { error } = await supabase.from("chapters").update({ subtitle }).eq("id", chapterId);
  if (error) throw error;
}
...
export async function markChapterComplete(chapterId, studentId) {
  if (!studentId || studentId === "null" || studentId === "undefined") {
    throw new Error("Impossible d'enregistrer la progression : aucun élève identifié.");
  }
  const { error } = await supabase.from("chapter_progress").insert({ chapter_id: chapterId, student_id: studentId });
  if (error && error.code !== "23505") {
    console.error("markChapterComplete failed:", error);
    throw new Error("Impossible d'enregistrer votre progression pour le moment. Réessayez.");
  }
}
...
export async function uploadChapterFile(file, courseId, chapterId) {
  // A clear, specific error beats a cryptic "Failed to fetch" — this
  // mirrors the bucket's actual limit (see fix-video-upload-schema.sql).
  const MAX_SIZE = 500 * 1024 * 1024; // 500MB
  if (file.size > MAX_SIZE) {
    throw new Error(`Ce fichier fait ${(file.size / 1024 / 1024).toFixed(0)}Mo — la limite est de 500Mo. Essayez de le compresser, ou utilisez plutôt un lien.`);
  }
  ...
  if (upErr) {
    // Surface the real Supabase error text instead of letting a generic
    // network failure reach the teacher with no explanation.
    throw new Error(upErr.message || "Échec de l'envoi du fichier. Vérifiez votre connexion et réessayez.");
  }
...
export async function addMediaContent(chapterId, { type, label, fileUrl }, sortOrder) {
  // type is "video" or "audio" — block_type must match it, or the
  // student viewer falls back to treating it as a text block (the
  // exact bug this fixes: block_type defaulted to 'explanation').
  const { error } = await supabase.from("chapter_content").insert({
    chapter_id: chapterId, type, block_type: type, label, file_url: fileUrl, sort_order: sortOrder,
  });
  if (error) throw error;
}
...
// Full attempt history for one student on one quiz — used for "best
// score", attempt count, and the chapter results panel. Real data only,
// never a placeholder: an empty array means the student never attempted it.
export async function fetchMyQuizAttempts(quizId, studentId) {
  const { data, error } = await supabase
    .from("quiz_submissions").select("*")
    .eq("quiz_id", quizId).eq("student_id", studentId)
...
export async function submitQuiz(quizId, studentId, score, total) {
  // Defense against exactly the reported bug class: never let a
  // missing/invalid id reach Postgres as a raw value.
  if (!studentId || studentId === "null" || studentId === "undefined") {
    throw new Error("Impossible d'enregistrer : aucun élève identifié. Ceci ne devrait jamais arriver pour un vrai élève connecté — contactez le support si le problème persiste.");
  }
  // Require a real, existing enrollment before writing any attempt —
  // this is what "confirm an enrollment exists" means in practice.
  const { data: quiz, error: quizErr } = await supabase.from("quizzes").select("course_id").eq("id", quizId).maybeSingle();
  if (quizErr || !quiz) throw new Error("Ce quiz est introuvable.");
  const { data: enrollment, error: enrErr } = await supabase
    .from("enrollments").select("id").eq("course_id", quiz.course_id).eq("student_id", studentId).maybeSingle();
  if (enrErr) throw new Error("Impossible de vérifier votre inscription pour le moment. Réessayez.");
  if (!enrollment) throw new Error("Vous devez être inscrit à ce cours pour soumettre ce quiz.");

  const { error } = await supabase.from("quiz_submissions").insert({ quiz_id: quizId, student_id: studentId, score, total });
  if (error) {
    console.error("submitQuiz failed:", error); // raw detail stays in the console for debugging
    throw new Error("Impossible d'enregistrer votre score pour le moment. Réessayez.");
  }
}
...
export async function issueCertificate(studentId, courseId, courseTitle, hoursSpent) {
  ...
  if (error) {
    ...
    throw new Error("Votre quiz est enregistré, mais le certificat n'a pas pu être généré. Contactez le support.");
  }
  return data;
}
...
export async function createChapterQuiz(courseId, chapterId, title, questions, passingScore = 70) {
  const { data: quiz, error } = await supabase
    .from("quizzes")
    .insert({ course_id: courseId, chapter_id: chapterId, title, is_final: false, passing_score: passingScore })
    .select().single();
  if (error) throw error;
  const rows = questions.map((q, i) => ({
    quiz_id: quiz.id, question: q.question, options: q.options, correct_answer: q.correctAnswer, sort_order: i,
...
// ---------- Public lesson preview (for sharing outside the app) ----------

export async function fetchPublicPreview(chapterId) {
  const { data, error } = await supabase
    .from("chapters")
    .select(`id, number, title, course:courses(id, title, description, cover_image_url, color, price)`)
    .eq("id", chapterId)
    .single();
  if (error) throw error;
  return data;
...
// ---------- Library (course documents — books, STL, and any file type) ----------

export async function fetchLibraryDocuments(studentId) {
  // Documents from every course the student is enrolled in
...
export async function fetchCourseDocuments(courseId) {
  const { data, error } = await supabase.from("documents").select("*").eq("course_id", courseId).order("uploaded_at", { ascending: false });
  if (error) throw error;
  return data || [];
}

export async function uploadCourseDocument(courseId, file, title, price) {
  const ext = file.name.split(".").pop();
  const path = `documents/${courseId}-${Date.now()}.${ext}`;
  const { error: upErr } = await supabase.storage.from("course-files").upload(path, file);
  if (upErr) throw upErr;
  const { data } = supabase.storage.from("course-files").getPublicUrl(path);
  const { error } = await supabase.from("documents").insert({
    course_id: courseId, title, file_url: data.publicUrl, file_name: file.name, price: price || 0,
  });
  if (error) throw error;
}
...
export async function generateDownloadPin(documentId, studentId) {
  const pin = generatePin();
  const { error } = await supabase.from("document_downloads").insert({ document_id: documentId, student_id: studentId, pin });
  if (error) throw error;
  return pin;
}

export async function fetchMyDownloadHistory(studentId) {
  const { data, error } = await supabase
...
// ---------- Admin: send a password reset link (never sees the real password) ----------
...
export async function fetchUserEmailsForAdmin() {
...
// ---------- Lesson blocks v2 (block types, images, quick-check, edit, reorder) ----------

export async function addTextBlock(chapterId, { label, body, blockType }, sortOrder) {
  const { error } = await supabase.from("chapter_content").insert({
    chapter_id: chapterId, type: "text", label, body, block_type: blockType, sort_order: sortOrder,
  });
  if (error) throw error;
}

export async function addImageBlock(chapterId, { label, fileUrl, altText }, sortOrder) {
  const { error } = await supabase.from("chapter_content").insert({
    chapter_id: chapterId, type: "image", label, file_url: fileUrl, alt_text: altText, block_type: "image", sort_order: sortOrder,
  });
  if (error) throw error;
}

export async function addQuizCheckBlock(chapterId, { label, question, options, correctAnswer }, sortOrder) {
  const { error } = await supabase.from("chapter_content").insert({
    chapter_id: chapterId, type: "text", label, block_type: "quiz_check",
    quiz_data: { question, options, correctAnswer }, sort_order: sortOrder,
  });
  if (error) throw error;
}

export async function updateChapterContentBlock(id, updates) {
  const { error } = await supabase.from("chapter_content").update(updates).eq("id", id);
  if (error) throw error;
}

export async function reorderChapterContentBlocks(idsInOrder) {
  // idsInOrder is the full array of content ids in their new order —
  // rewrite sort_order 0..n for all of them in one batch.
  const updates = idsInOrder.map((id, i) => supabase.from("chapter_content").update({ sort_order: i }).eq("id", id));
  const results = await Promise.all(updates);
  const failed = results.find((r) => r.error);
  if (failed) throw failed.error;
}

export async function setChapterPublished(chapterId, published) {
  const { error } = await supabase.from("chapters").update({ published }).eq("id", chapterId);
  if (error) throw error;
}

// ---------- Chapter unlocking (quiz-gated progression) ----------

export async function fetchUnlockedChapters(courseId) {
  const { data, error } = await supabase.rpc("get_unlocked_chapters", { p_course_id: courseId });
  if (error) throw error;
  const map = {};
  (data || []).forEach((row) => { map[row.chapter_id] = row.unlocked; });
  return map;
}

export async function updateQuizPassingScore(quizId, passingScore) {
  const { error } = await supabase.from("quizzes").update({ passing_score: passingScore }).eq("id", quizId);
  if (error) throw error;
}

// One batched call for the whole chapter list — avoids querying per
// chapter. Returns { [chapterId]: { bestPct, latestPct, attempts, passed, passingScore } }
// Real scores only — a chapter absent from the map means "never attempted".
export async function fetchChapterScores(courseId, studentId) {
  const { data: quizzes, error: qErr } = await supabase
    .from("quizzes").select("id, chapter_id, passing_score")
    .eq("course_id", courseId).eq("is_final", false).not("chapter_id", "is", null);
  if (qErr) throw qErr;
  ...
  const { data: subs, error: sErr } = await supabase
    .from("quiz_submissions").select("*")
    .in("quiz_id", quizIds).eq("student_id", studentId)
    .order("submitted_at", { ascending: false });
  if (sErr) throw sErr;

  const result = {};
  quizzes.forEach((quiz) => {
    const mine = (subs || []).filter((s) => s.quiz_id === quiz.id);
    if (mine.length === 0) return;
    const pcts = mine.map((s) => (s.total > 0 ? Math.round((s.score / s.total) * 100) : 0));
    result[quiz.chapter_id] = {
      bestPct: Math.max(...pcts),
      latestPct: pcts[0], // already sorted newest-first
      attempts: mine.length,
      passed: pcts.some((p) => p >= (quiz.passing_score ?? 70)),
...
export async function fetchChapterCompletionDate(chapterId, studentId) {
  const { data, error } = await supabase
    .from("chapter_progress").select("completed_at")
    .eq("chapter_id", chapterId).eq("student_id", studentId).maybeSingle();
  if (error) throw error;
