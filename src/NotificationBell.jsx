...
export default function NotificationBell({ userId, lastSeen, color = "#7C6FFF" }) {
  ...
  return (
    <div ref={menuRef} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: 44, right: 0, background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 14, width: 320, maxHeight: 420, overflowY: "auto", zIndex: 200, boxShadow: "0 20px 60px #000c" }}>
            ...
            return (
              <div key={a.id} style={{ padding: "10px 14px", borderBottom: "1px solid #E2E5EB", background: isUnread ? color + "0A" : "transparent" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                  ...
                  <span style={{ fontWeight: 700, fontSize: 12, color: "#12141C" }}>{a.title?.trim() || "(sans titre)"}</span>
                </div>
                <p style={{ margin: "0 0 3px", fontSize: 11, color: "#4B5568", lineHeight: 1.5 }}>{a.message?.trim() || "(message vide)"}</p>
