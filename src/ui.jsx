...
export function Modal({ title, onClose, children, width = 500, noPadding = false }) {
  return (
    <div className="modal-overlay" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.82)", backdropFilter: "blur(8px)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center" }}
      onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-panel" style={{ background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 16, width: "100%", maxWidth: width, overflow: "auto", padding: noPadding ? 0 : 26, boxShadow: "0 40px 100px #000a", position: "relative" }}>
        {noPadding ? (
          <button onClick={onClose} style={{ position: "absolute", top: 14, right: 14, background: "rgba(255,255,255,.9)", border: "none", borderRadius: "50%", width: 30, height: 30, color: "#12141C", fontSize: 16, cursor: "pointer", zIndex: 1, boxShadow: "0 2px 8px rgba(0,0,0,.15)" }}>✕</button>
        ) : (
      ...
      <style>{`
        .modal-overlay { padding: max(20px, var(--safe-top)) max(20px, var(--safe-right)) max(20px, var(--safe-bottom)) max(20px, var(--safe-left)); box-sizing: border-box; }
        .modal-panel { max-height: 90vh; max-height: 90dvh; box-sizing: border-box; }
      `}</style>
    </div>
  );
}
...
export function NavBar({ user, tabs, active, onTab, onLogout, onProfile, avatarImg, userId, lastSeenAnnouncements }) {
  ...
  return (
    <div style={{ background: "#FFFFFF", borderBottom: "1px solid #E2E5EB", flexShrink: 0, position: "relative", paddingTop: "env(safe-area-inset-top)" }}>
      <div className="nav-inner" style={{ padding: "0 14px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div ref={menuRef} style={{ position: "relative" }}>
            <button className="nav-touch-target" onClick={() => setOpen((v) => !v)} style={{
          ...
          <img src="/logo.png" alt="TeknikSchool" style={{ width: 30, height: 30, objectFit: "contain", flexShrink: 0 }} />
          <span className="nav-role-badge"><Badge label={user.role} color={c} /></span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {userId && <NotificationBell userId={userId} lastSeen={lastSeenAnnouncements} color={c} />}
          <div ref={avatarRef} style={{ position: "relative" }}>
            <button className="nav-touch-target" onClick={() => setAvatarOpen((v) => !v)} style={{ display: "flex", alignItems: "center", gap: 10, background: "#F7F8FA", border: "1px solid #E2E5EB", cursor: "pointer", padding: "0 10px 0 0", borderRadius: 24 }}>
              <Av name={user.name} size={32} color={c} img={avatarImg} />
              <span className="nav-user-name" style={{ fontSize: 13, color: "#12141C", fontWeight: 600 }}>{user.name}</span>
            </button>
            {avatarOpen && (
              <div style={{ position: "absolute", top: 50, right: 0, background: "#FFFFFF", border: "1px solid #E2E5EB", borderRadius: 14, padding: 14, minWidth: 190, zIndex: 100, boxShadow: "0 20px 60px #000c", animation: "navIn .15s ease" }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#12141C", marginBottom: 4 }}>{user.name}</div>
                <div style={{ marginBottom: 12 }}><Badge label={user.role} color={c} /></div>
                {onProfile && (
                  <button onClick={() => { onProfile(); setAvatarOpen(false); }} style={{ width: "100%", background: "none", border: "1px solid #E2E5EB", borderRadius: 9, padding: "9px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 9, color: "#1F2430", fontWeight: 600, fontSize: 13, fontFamily: "'Syne',sans-serif", textAlign: "left", marginBottom: 6 }}>
                    <UserIcon size={15} /><span>Mon profil</span>
                  </button>
                )}
                <button onClick={() => { onLogout(); setAvatarOpen(false); }} style={{ width: "100%", background: "none", border: "1px solid #FF667755", borderRadius: 9, padding: "9px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 9, color: "#FF6677", fontWeight: 600, fontSize: 13, fontFamily: "'Syne',sans-serif", textAlign: "left" }}>
                  <LogOutIcon size={15} /><span>Déconnexion</span>
                </button>
              </div>
            )}
          </div>
      ...
      <style>{`
        ...
        .nav-touch-target{width:44px;height:44px}
        @media (max-width: 480px) {
          .nav-role-badge, .nav-user-name { display: none; }
        }
      `}</style>
    </div>
  );
